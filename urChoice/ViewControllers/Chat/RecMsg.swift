...
//        UIHelper.shared.setImage(address: other.image , imgView: userImage)
        ...
        else if item.messageType == "image"
        {
            ...
            audioMsgUrl = item.user_message
            let time = (Double(100) ?? 0.0) / 1000
        ...
        userName.text       = item.user_fullname
        UIHelper.shared.setImage(address: item.user_image_url ?? "", imgView: userImage)
        let time            = Date()
